...
import { getAuthor, getAuthorAvatar } from '../composables/useAuthors'
// Hero 背景
...
const carouselAuthors = computed(() => {
  ...
  return targetAuthors.map((name) => {
    const authorInfo = getAuthor(name)
    const artworks = authorArtworks.value.get(name) || []
    const topArtworks = artworks.slice(0, 8)
    return {
      name,
      pinyin: authorInfo?.pinyin || name.toUpperCase(),
      subtitle: authorInfo?.subtitle || '',
      subtitlePinyin: authorInfo?.subtitlePinyin || '',
